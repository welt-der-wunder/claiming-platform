...
<div class="card">
  <div class="mx-3 p-2 d-flex text-end justify-content-end">
    <div class="mx-1">
      <form action="{{ route('exportData') }}" method="post">
        @csrf
        <input type="hidden" name="url_params"
          value="<?php if(isset($_SERVER['QUERY_STRING'])) echo $_SERVER['QUERY_STRING']; ?>" />
    ...
    <div class="mx-1">
      <button type="button" class="btn btn-primary m-1 fade show" data-bs-toggle="modal"
  ...
  <div class="modal-dialog rounded" style="top: 50%; transform: translateY(-50%);">
    <div class="modal-content" style="background: #1d162a;">
      <div class="modal-header">
        <h5 class="modal-title" id="fileImportModalLabel">{{__('Import list')}}</h5>
        <button type="button" class="btn-close rounded-circle" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <!-- Add your form fields for creating a new record here -->
        <form action="{{ route('importData') }}" method="POST" id="importForm" enctype="multipart/form-data">
          @csrf
