      ...
      <div class="row col-md-6 my-1">
        <div class="col-6">
          <select id="" class="form-select" name="reward_status" onchange="submit()">
                ...
                <option value="{{ $status }}" @if (isset($_GET['reward_status']) && $_GET['reward_status'] == $status) {{"selected"}} @endif>{{__( $status )}}</option>
                @endforeach
          </select>
        </div>
    ...
    <form action="{{ route('processMultipleRewards') }}" method="post">
      @csrf
       <div class="mx-3 text-end">
        <button type="submit" name="confirm" value="1" class="btn btn-primary m-1 fade show d-none" id="confirmBtn" onclick="return confirmAction();">{{__('Confirm')}}</button>
        <button type="submit" name="reject" value="1" class="btn btn-primary m-2 fade show d-none" id="rejectBtn" onclick="return confirmAction();">{{__('Reject')}}</button>
        <button type="submit" name="pending" value="1" class="btn btn-primary m-2 fade show d-none" id="pendingBtn" onclick="return confirmAction();">{{__('Move Back')}}</button>
       </div>
      <table class="table">
        <thead>
          <tr>
        ...
        <tbody class="table-border-bottom-0">
          ...
          <tr>
            <td align="center">
              <input onchange="updateButtons()" class="form-check-input w-10 h-10" name="user_ids[]" type="checkbox" value="{{$user->id}}">
            </td>
            <td>{{$user->id}}</td>
            ...
            <td align="center"><a class="action-link-c" href="{{ route('showTimeline', $user->id) }}"><i class='bx bxs-show text-wite' ></i></a></td>
          </tr>
          @endforeach
        </tbody>
        @else
